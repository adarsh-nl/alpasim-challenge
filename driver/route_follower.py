#!/usr/bin/env python
"""Route-following driver: no neural net. Tracks the route centerline.

Gates to clear:  collision_at_fault == 0  (front|lateral),  offroad == 0
Then:            score = min(progress_clipped_rel / 0.8, 1.0)
"""
...
LOG = logging.getLogger("route_follower")

V_MAX = float(os.environ.get("RF_V_MAX", "12.0"))       # m/s cap
A_LAT_MAX = float(os.environ.get("RF_A_LAT", "2.5"))    # m/s^2 lateral
A_LON_MAX = float(os.environ.get("RF_A_LON", "1.5"))    # m/s^2 accel
TAU_CONVERGE = 1.5                                      # s, lateral re-centering
...
class Session:
    ...
    speed: float = 0.0
    route_rig: np.ndarray | None = None      # (N,3) rig frame
    route_ts: int = 0
    ...
    def add_pose(self, p: common_pb2.PoseAtTime) -> None:
        ...
        if self.latest is None or p.timestamp_us >= self.latest.timestamp_us:
            self.latest = p
        while len(self.pose_ts) > 400:                  # bound memory
...
def resample(pts: np.ndarray, step: float = 0.5) -> tuple[np.ndarray, np.ndarray]:
    """Densify a polyline; return (points, cumulative arclength)."""
...
class RouteFollower(egodriver_pb2_grpc.EgodriverServiceServicer):
    ...
    def start_session(self, req, ctx):
        ...
        LOG.info("start %s", req.session_uuid)
    ...
    def _plan(self, s: Session, t0: int) -> common_pb2.Trajectory:
        ...
        route = np.vstack([p0[None, :], route])          # anchor plan at ego
        pts, s_arc = resample(route)
        if len(pts) < 2:
            return straight(p0, yaw0, t0, max(s.speed, 3.0))

        # project ego, get lateral error
        ...
        # curvature-limited speed profile
    ...
    def get_version(self, req, ctx):
        return common_pb2.VersionId(version_id="route-follower",
                                    git_hash="local",
...
def fallback(s: Session, t0: int) -> common_pb2.Trajectory:
    """Decelerate along current heading. Never throw."""
    if s.latest is None:
...
def main() -> None:
    ...
    srv.start()
    LOG.info("route-follower on %s:%d  V_MAX=%.1f", host, port, V_MAX)
